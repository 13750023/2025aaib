...
PImage [] toys = new PImage[200];//Java 的陣列,開很多!
int N = 0;//現在有N張小圖片在使用
int [] x = new int[200];//200個圖片座標
int [] y = new int[200];//200個圖片座標
...
void draw(){
  ...
  for(int i=0; i<N; i++){//針對每張小圖片照座標貼上來
    image(toy[i], x[i]-25, y[i]-25);
  }
  //if(mousePressed && toy3 != null) image(toy3, mouseX-25, mouseY-25); //再秀第2張圖
}
void mousePressed(){
  x[N] = mouseX;
  y[N] = mouseY;
  toy[N] = toy2.get(mouseX-25, mouseY-25, 50, 50);
  N++;
